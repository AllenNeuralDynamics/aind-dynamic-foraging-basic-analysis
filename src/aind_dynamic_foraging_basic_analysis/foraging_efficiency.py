...
For the coupled-block-with-baiting task (Jeremiah's 2019 Neuron paper), 
...
For the non-baiting task (Cooper Grossman), I assume the ideal observer knows the underlying probability and 
makes the greedy choice. To account for the randomness of each actual session, I simulated the ideal observers 
using the actual random seed I used during the experiment.
...
at least in an average sense. For a single session, however, there are chances where the efficiency
can be larger than 1 because of the randomness of the task (sometimes the mice are really 
lucky that they get more reward than performing "optimally")
"""
