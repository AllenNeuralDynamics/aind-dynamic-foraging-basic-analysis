...
def session_metrics(nwb):
    ...
    p_larger_global = max(
        df_trial["probability_chosen"].max(), df_trial["probability_not_chosen"].max()
    )

    p_smaller_global = min(
        df_trial["probability_chosen"].min(), df_trial["probability_not_chosen"].min()
    )
    ...
    dict_meta = {
        ...
        "double_dipping_rate_finished_trials": (
            df_trial.loc[(df_trial.animal_response != IGNORE), "n_lick_switches_gocue_stop"] > 0
        ).sum()
        / (df_trial.animal_response != IGNORE).sum(),
        "double_dipping_rate_finished_reward_trials": (
            df_trial.loc[df_trial.reward, "n_lick_switches_gocue_stop"] > 0
        ).sum()
        / df_trial.reward.sum(),
        "double_dipping_rate_finished_noreward_trials": (
            df_trial.loc[
                (df_trial.animal_response != IGNORE) & (~df_trial.reward),
        ...
        "lick_consistency_mean_finished_trials": df_trial.loc[
            (df_trial.animal_response != IGNORE), "n_lick_consistency_gocue_stop"
        ].mean(),
        "lick_consistency_mean_finished_reward_trials": df_trial.loc[
            df_trial.reward, "n_lick_consistency_gocue_stop"
        ].mean(),
        "lick_consistency_mean_finished_noreward_trials": df_trial.loc[
            (df_trial.animal_response != IGNORE) & (~df_trial.reward),
