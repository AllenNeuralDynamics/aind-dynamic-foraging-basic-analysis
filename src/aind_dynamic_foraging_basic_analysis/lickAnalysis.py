...
def plotLickAnalysis(nwb):
    ...
    gs = gridspec.GridSpec(3, 6, width_ratios=np.ones(6).tolist(), height_ratios=np.ones(3).tolist())
    ...
    ax.hist(1000*allLickDiffs[(allLicksID[:-1]==0) & (allLicksID[1:]==0)], bins=np.arange(0, 2500, 20))
    ax.set_title('ILI_L-L')
    ax.set_xlabel('ms')
    ax = fig.add_subplot(gs[0, 1])
    ax.hist(1000*allLickDiffs[(allLicksID[:-1]==1) & (allLicksID[1:]==1)], bins=np.arange(0, 2500, 20))
    ax.set_title('ILI_R-R')
    ax.set_xlabel('ms')
    ax = fig.add_subplot(gs[0, 2])
    ax.hist(1000*allLickDiffs[(allLicksID[:-1]==1) & (allLicksID[1:]==0)], bins=np.arange(0, 2500, 20))
    ax.set_title('ILI_R-L')
    ax.set_xlabel('ms')
    ax = fig.add_subplot(gs[0, 3])
    ax.hist(1000*allLickDiffs[(allLicksID[:-1]==0) & (allLicksID[1:]==1)], bins=np.arange(0, 2500, 20))
    ...
    lickDelay = tblTrials['goCue_start_time'] - tblTrials['start_time'] - tblTrials['ITI_duration']
    ...
    sortIndL = np.argsort(tblTrials.loc[tblTrials['animal_response']==0, 'delay_duration'])
    sortIndR = np.argsort(tblTrials.loc[tblTrials['animal_response']==1, 'delay_duration'])
    LAlign = tblTrials.loc[tblTrials['animal_response']==0, 'goCue_start_time'].values
    RAlign = tblTrials.loc[tblTrials['animal_response']==1, 'goCue_start_time'].values
    ...
    if len(LAlign)>0:
        ...
        ax.scatter(-np.sort(tblTrials.loc[tblTrials['animal_response']==0, 'delay_duration']), range(len(tblTrials.loc[tblTrials['animal_response']==0, 'delay_max'])), c='b', marker= '|', s=1, zorder = 1)
        ax.axvline(x = 0, c = 'r', ls = '--', lw = 1, zorder = 3)
        ax.set_title('L licks on L choice trials')

        ax = fig.add_subplot(gs[2, 0])
        countsPre = np.searchsorted(np.sort(df.time.values), edges-0.5*binSize)
        countsPost = np.searchsorted(np.sort(df.time.values), edges+0.5*binSize)
    ...
    if len(RAlign)>0:
        ...
        ax.scatter(df.time, df.event_index, c='k', marker= '|', s=1, zorder = 2)
        ax.scatter(-np.sort(tblTrials.loc[tblTrials['animal_response']==1, 'delay_duration']), range(len(tblTrials.loc[tblTrials['animal_response']==1, 'delay_max'])), c='b', marker= '|', s=1, zorder = 1)
        ax.axvline(x = 0, c = 'r', ls = '--', lw = 1, zorder = 3)
