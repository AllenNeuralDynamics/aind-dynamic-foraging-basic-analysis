...
def plot_lick_analysis(nwb):
    ...
    all_licks_id = np.concatenate(
        (np.ones_like(right_licks), np.zeros_like(left_licks))
    )
    ...
    l_align = tbl_trials.loc[
        tbl_trials["animal_response"] == 0, "goCue_start_time"
    ].values
    r_align = tbl_trials.loc[
        tbl_trials["animal_response"] == 1, "goCue_start_time"
    ].values
    ...
    if len(l_align) > 0:
        ...
        ax1.scatter(
            -np.sort(
                tbl_trials.loc[
                    tbl_trials["animal_response"] == 0, "delay_duration"
                ]
            ),
            range(
                len(
                    tbl_trials.loc[
                        tbl_trials["animal_response"] == 0, "delay_max"
                    ]
                )
            ),
            c="b",
    ...
    if len(r_align) > 0:
        ...
        ax1.scatter(
            -np.sort(
                tbl_trials.loc[
                    tbl_trials["animal_response"] == 1, "delay_duration"
                ]
            ),
            range(
                len(
                    tbl_trials.loc[
                        tbl_trials["animal_response"] == 1, "delay_max"
                    ]
                )
            ),
            c="b",
    ...
    lick_lat = (
        tbl_trials["reward_outcome_time"] - tbl_trials["goCue_start_time"]
    )
...
def cal_metrics(data):
    ...
    lick_percent_l = [
        np.sum(lick_lat_l <= thresh_curr) / np.shape(lick_lat_l)[0]
        for thresh_curr in thresh
    ]
    lick_percent_r = [
        np.sum(lick_lat_r <= thresh_curr) / np.shape(lick_lat_r)[0]
        for thresh_curr in thresh
    ]
    ...
    finish_kernel = finish_kernel[
        int(0.5 * len(kernel)): -int(0.5 * len(kernel))
    ]
    ...
    all_pre_nolick = (
        tbl_trials.loc[
            tbl_trials["animal_response"] != 2, "goCue_start_time"
        ].values
        - tbl_trials.loc[
            tbl_trials["animal_response"] != 2, "delay_duration"
        ].values
    )
...
def plot_met(data, lick_met):
    ...
    ax1.hist(
        lick_lat_l, bins=edges, alpha=0.5, density=True, label="lick hist"
    )
    ax1.set_xlabel("s")
    ...
    all_go_rwd = tbl_trials.loc[
        (tbl_trials["animal_response"] != 2)
        & (tbl_trials["rewarded_historyL"] == 1)
...
def plot_raster_rate(
    ...
    counts_pre = np.searchsorted(
        np.sort(df.time.values), edges - 0.5 * bin_size
    )
    counts_post = np.searchsorted(
        np.sort(df.time.values), edges + 0.5 * bin_size
    )
