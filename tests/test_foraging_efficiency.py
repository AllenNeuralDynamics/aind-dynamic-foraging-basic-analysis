...
def get_history_from_nwb(nwb_file):
    ...
    reward_history = df_trial.rewarded_historyL[non_autowater] | df_trial.rewarded_historyR[non_autowater]
    reward_probability = [df_trial.reward_probabilityL[non_autowater].values, 
                            df_trial.reward_probabilityR[non_autowater].values]
    random_number = [df_trial.reward_random_number_left[non_autowater].values, 
...
class TestForagingEfficiency(unittest.TestCase):
    ...
    def test_example_sessions(self):
        ...
        for correct_answer in self.correct_answers:
            ...
            choice_history, reward_history, reward_probability, random_number = get_history_from_nwb(
                nwb_file)
            foraging_efficiency, foraging_efficiency_random_seed = compute_foraging_efficiency(
                choice_history, reward_history, reward_probability, random_number)
            self.assertAlmostEqual(
                foraging_efficiency,
                correct_answer['foraging_efficiency']
            )
            self.assertAlmostEqual(
    ...
    def test_wrong_format(self):
        ...
        with self.assertRaises(ValueError):
            compute_foraging_efficiency(choice_history, reward_history, reward_probability, random_number)

        compute_foraging_efficiency(choice_history, reward_history, reward_probability) # This should work
